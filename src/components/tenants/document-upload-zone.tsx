...
import { useUploadDocument } from "@/hooks/queries/use-tenants"
import { getErrorMessage } from "@/lib/get-error-message"
import { DOCUMENT_TYPE_LABELS, DOCUMENT_TYPES_BY_TENANT_TYPE } from "@/lib/tenant-labels"
import { cn } from "@/lib/utils"
import type { DocumentType, TenantType } from "@/types/tenant"
...
export function DocumentUploadZone({ tenantId, tenantType }: { tenantId: string; tenantType: TenantType }) {
  const uploadMutation = useUploadDocument(tenantId)
  const availableTypes = DOCUMENT_TYPES_BY_TENANT_TYPE[tenantType]
  const [documentType, setDocumentType] = React.useState<DocumentType>(availableTypes[0])
  const [isDragging, setIsDragging] = React.useState(false)
  const [pending, setPending] = React.useState<PendingUpload[]>([])
  const fileInputRef = React.useRef<HTMLInputElement>(null)

  function startUpload(file: File, docType: DocumentType, existingId?: string) {
