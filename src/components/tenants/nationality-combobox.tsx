...
export function NationalityCombobox({
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        <span className={cn("truncate", !value && "text-outline")}>
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full overflow-hidden rounded-lg border border-outline-variant bg-surface shadow-md">
          ...
          <div className="max-h-60 overflow-y-auto p-1">
            {filtered.length === 0 ? (
              <p className="px-3 py-4 text-center text-body-md text-on-surface-variant">No countries found.</p>
            ) : (
              filtered.map((country) => (
                ...
                >
                  <span>{country}</span>
                  {country === value && <Check className="h-4 w-4 text-secondary" />}
                </button>
              ))
            )}
          </div>
