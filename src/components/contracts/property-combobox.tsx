...
function propertyLabel(property: Property) {
  return `Unit ${property.unitNumber} — ${property.building?.name ?? "Unknown building"}`
}
...
export function PropertyCombobox({
  ...
  const filtered = query.trim()
    ? properties.filter((property) =>
        `${property.unitNumber} ${property.building?.name ?? ""} ${property.building?.code ?? ""}`
          .toLowerCase()
    ...
    <div ref={containerRef} className="relative">
      ...
      >
        <span className={cn("truncate", !selected && "text-outline")}>
          {selected ? propertyLabel(selected) : isLoading ? "Loading properties…" : "Select a property…"}
        </span>
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full overflow-hidden rounded-lg border border-outline-variant bg-surface shadow-md">
          ...
          <div className="max-h-60 overflow-y-auto p-1">
            {filtered.length === 0 ? (
              <p className="px-3 py-4 text-center text-body-md text-on-surface-variant">No properties found.</p>
            ) : (
              filtered.map((property) => (
                ...
                >
                  ...
                  {property.id === value && <Check className="h-4 w-4 text-secondary" />}
                </button>
              ))
            )}
          </div>
