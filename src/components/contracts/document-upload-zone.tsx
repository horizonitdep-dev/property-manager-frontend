...
import { useUploadContractDocument } from "@/hooks/queries/use-contracts"
import { getErrorMessage } from "@/lib/get-error-message"
import { CONTRACT_DOCUMENT_TYPE_LABELS, CONTRACT_DOCUMENT_TYPE_OPTIONS } from "@/lib/contract-labels"
...
export function DocumentUploadZone({ contractId }: { contractId: string }) {
  const uploadMutation = useUploadContractDocument(contractId)
  const [documentType, setDocumentType] = React.useState<ContractDocumentType>(CONTRACT_DOCUMENT_TYPE_OPTIONS[0])
  const [isDragging, setIsDragging] = React.useState(false)
  const [pending, setPending] = React.useState<PendingUpload[]>([])
  const fileInputRef = React.useRef<HTMLInputElement>(null)

  function startUpload(file: File, docType: ContractDocumentType, existingId?: string) {
    const id = existingId ?? crypto.randomUUID()
    setPending((prev) => [...prev.filter((p) => p.id !== id), { id, file, documentType: docType, progress: 0, status: "uploading" }])
  ...
  return (
    <div className="space-y-4">
      <div className="max-w-xs space-y-2">
        <label className="text-label-sm uppercase text-on-surface-variant">Document type for next upload</label>
        <Select value={documentType} onValueChange={(value) => setDocumentType(value as ContractDocumentType)}>
          <SelectTrigger>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {CONTRACT_DOCUMENT_TYPE_OPTIONS.map((type) => (
      ...
      >
        ...
        <p className="text-body-md text-on-surface">
          Drag &amp; drop files here, or <span className="font-bold text-secondary">browse</span>
        </p>
              ...
              <div className="flex shrink-0 items-center gap-1">
                {item.status === "error" && (
                  <Button
                    ...
                    onClick={() => startUpload(item.file, item.documentType, item.id)}
                  >
