...
import { useUploadFinanceAttachment } from "@/hooks/queries/use-finance-attachments"
...
export function AttachmentUploadZone({
  ...
  parentId: string
  /** Matches the server-side default for this parent (payment → RECEIPT, etc). */
  defaultType: FinanceAttachmentType
  prompt: string
}) {
  const uploadMutation = useUploadFinanceAttachment(parent, parentId)
  const [attachmentType, setAttachmentType] = React.useState<FinanceAttachmentType>(defaultType)
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-body-md text-on-surface">
          {prompt}, or <span className="font-bold text-secondary">browse</span>
        </p>
