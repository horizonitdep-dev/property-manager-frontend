...
import { AttachmentUploadZone } from "@/components/finance/attachment-upload-zone"
import { DeleteAttachmentDialog } from "@/components/finance/delete-attachment-dialog"
import { RoleGate } from "@/components/role-gate"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { Skeleton } from "@/components/ui/skeleton"
import { useFinanceAttachments } from "@/hooks/queries/use-finance-attachments"
...
import type { FinanceAttachment, FinanceAttachmentType } from "@/types/finance"
...
export function AttachmentsSection({
  ...
  parentId: string
  title: string
  defaultType: FinanceAttachmentType
  uploadPrompt: string
}) {
  const attachmentsQuery = useFinanceAttachments(parent, parentId)
  ...
  async function handleView(attachment: FinanceAttachment) {
    ...
    try {
      const { url } = await financeAttachmentService.getSignedUrl(parent, parentId, attachment.id)
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-outline-variant bg-surface">
      <div className="flex items-center gap-2 border-b border-outline-variant bg-surface-container-low px-6 py-4">
        ...
        <h3 className="font-display text-h2 text-on-surface">{title}</h3>
      </div>
