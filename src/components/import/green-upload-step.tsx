...
import { useValidateGreenContractImport } from "@/hooks/queries/use-green-contract-import"
import { getErrorMessage } from "@/lib/get-error-message"
import { cn } from "@/lib/utils"
import type { GreenContractImportSession } from "@/types/import-green-contract"
...
/**
 * A clone of PdfUploadStep wired to the Green endpoint — the DMT copy is not
 * parameterized and must stay untouched. Same honesty about progress: one
 * multipart POST has no per-file server progress, so the counter eases up
 * toward the total without ever claiming completion.
 */
export function GreenUploadStep({
  onValidated,
}: {
  onValidated: (session: GreenContractImportSession) => void
}) {
  const shouldReduceMotion = useReducedMotion()
  const validateMutation = useValidateGreenContractImport()
  ...
  React.useEffect(() => {
    if (!isBusy || pending.length === 0) {
      setExtractedCount(0)
      return
    }
    const interval = setInterval(() => {
      setExtractedCount((prev) => Math.min(prev + 1, Math.max(pending.length - 1, 0)))
    }, 2500)
    return () => clearInterval(interval)
  }, [isBusy, pending.length])
  ...
  return (
    <div className="space-y-4">
      <p className="text-body-md text-on-surface-variant">
        Upload the client&apos;s internal tenancy contracts. Fields will be extracted automatically for your
