...
import { useCommitGreenContractImport } from "@/hooks/queries/use-green-contract-import"
...
/** Mirrors PdfConfirmStep, wired to the Green commit endpoint (sessionId only). */
export function GreenConfirmStep({
  session,
  onBack,
  onCommitted,
  onClose,
}: {
  session: GreenContractImportSession
  onBack: () => void
  onCommitted: () => void
  onClose: () => void
}) {
  const commitMutation = useCommitGreenContractImport()
  ...
  const blocked = session.summary.blockedContracts

  function handleConfirm() {
    commitMutation.mutate(session.sessionId, { onSuccess: () => onCommitted() })
  }
        ...
        >
          <div className="rounded-xl border border-outline-variant bg-surface-container-low p-6 text-center">
            ...
            <p className="mt-2 text-body-md text-on-surface-variant">
              Will create {buildings} building(s), {properties} properties, {tenants} tenants, {contracts}{" "}
              contracts
              {blocked > 0 && ` (${blocked} skipped — unit already has a contract)`}.
            </p>
          </div>
          ...
          <div className="flex items-center justify-between border-t border-outline-variant pt-4">
            ...
            <Button
              ...
              disabled={commitMutation.isPending || contracts === 0}
            >
          ...
          {contracts === 0 && (
            <p className="text-right text-sm text-error">
              No importable contracts — every uploaded unit already has a contract on file.
            </p>
          )}
        </motion.div>
      )}
